...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["file_excel"])) {
    ...
    for ($row = 2; $row <= count($sheetData); $row++) {
        $id_anggota  = $sheetData[$row]['A'];
        $username = $sheetData[$row]['B'];
        $password = md5($sheetData[$row]['C']);
        $nama = $sheetData[$row]['D'];
        $jekel = $sheetData[$row]['E'];
        $kelas = $sheetData[$row]['F'];
        $no_hp = $sheetData[$row]['G'];

        // Masukkan data ke dalam database
        $sql = "INSERT INTO tb_anggota (id_anggota , username, password, nama, jekel, kelas, no_hp) VALUES ('$id_anggota ', '$username', '$password', '$nama', '$jekel', '$kelas', '$no_hp')";
        if ($mysqli->query($sql) !== true) {
            echo "Error: " . $sql . "<br>" . $mysqli->error;
        }
    }

    echo "<script>alert('Data berhasil ditambah'); document.location='index.php?page=MyApp/data_agt';</script>";
}
    ...
    <div class='col-md-12'>
        <div class='box box-info'>
            <div class='box-header with-border'>
                <h3 class='box-title'>Tambah Anggota Perpustakaan </h3>
            </div>
            <div class='box-body'>
                <div class="alert alert-info mb-5" role="alert">
                    ...
                    <ul>
                        <li>
                            ...
                            <a class="font-weight-bold" target="_blank" href="uploads/Data_Anggota.xlsx">
